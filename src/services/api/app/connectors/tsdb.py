...
_query_last = f"""from(bucket: "{CONFIG.TSDB_BUCKET}") |> range(start: -15m, stop: now()) |> tail(n: 1)"""  # noqa: E501
_query_avg_1d = f"""from(bucket: "{CONFIG.TSDB_BUCKET}") |> range(start: -1d, stop: now()) |> mean() |> group()"""  # noqa: E501
_query_avg_1mo = f"""from(bucket: "{CONFIG.TSDB_BUCKET}") |> range(start: -1mo, stop: now()) |> mean() |> group()"""  # noqa: E501
...
def _query_storage(query):
    ...
    return query_api.query(query=query, org=CONFIG.TSDB_ORG).to_json(
        columns=["currency", "_value", "_time"]
    )
